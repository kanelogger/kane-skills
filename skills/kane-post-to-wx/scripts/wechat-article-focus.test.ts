...
test("legacy browser article saves only when submit is requested", () => {
  assert.match(articleScript, /if \(submit\) \{[\s\S]*?Saving as draft/);
  assert.match(articleScript, /Article composed \(preview mode\)\. Add --submit/);
});
