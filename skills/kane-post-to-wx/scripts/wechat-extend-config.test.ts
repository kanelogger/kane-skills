...
async function writeEnvFile(root: string, content: string): Promise<void> {
  const envPath = path.join(root, ".kane-skills", ".env");
  await fs.mkdir(path.dirname(envPath), { recursive: true });
  await fs.writeFile(envPath, content);
}

async function writeExtendFile(root: string, content: string): Promise<void> {
  const extendPath = path.join(root, ".kane-skills", "kane-post-to-wx", "EXTEND.md");
...
test("loadCredentials selects the first complete source without mixing values across sources", async (t) => {
  ...
  assert.equal(credentials.source, "<cwd>/.kane-skills/.env");
...
test("loadCredentials reports skipped incomplete sources when no complete pair exists", async (t) => {
  ...
  assert.throws(
    () => loadCredentials(),
    /Incomplete credential sources skipped:\n- process\.env missing WECHAT_APP_SECRET\n- <cwd>\/\.kane-skills\/\.env missing WECHAT_APP_ID/,
  );
});

test("loadCredentials keeps legacy credential files working after the Kane rename", async (t) => {
  const cwdRoot = await makeTempDir("wechat-creds-legacy-cwd-");
  const homeRoot = await makeTempDir("wechat-creds-legacy-home-");
  ...
  const legacyPath = path.join(cwdRoot, ".baoyu-skills", ".env");
  await fs.mkdir(path.dirname(legacyPath), { recursive: true });
  await fs.writeFile(legacyPath, "WECHAT_APP_ID=legacy-id\nWECHAT_APP_SECRET=legacy-secret\n");

  const credentials = loadCredentials();
  assert.equal(credentials.appId, "legacy-id");
  assert.equal(credentials.appSecret, "legacy-secret");
  assert.equal(credentials.source, "legacy project config");
});

test("loadWechatExtendConfig keeps legacy preference files working after the Kane rename", async (t) => {
  const cwdRoot = await makeTempDir("wechat-extend-legacy-cwd-");
  const homeRoot = await makeTempDir("wechat-extend-legacy-home-");
  ...
  await fs.mkdir(path.dirname(legacyPath), { recursive: true });
  await fs.writeFile(legacyPath, "default_author: Legacy Author\n");

  assert.equal(loadWechatExtendConfig().default_author, "Legacy Author");
});

test("resolveRenderStyle uses the personal profile by default", () => {
  const style = resolveRenderStyle({});

  assert.equal(style.profile, "personal");
  assert.equal(style.theme, "default");
  assert.equal(style.color, "blue");
  assert.equal(style.fontSize, "16px");
  assert.equal(style.lineHeight, "1.75");
  assert.equal(style.headingStyle, "filled");
  assert.equal(style.blockquoteStyle, "soft");
});

test("loadWechatExtendConfig applies case-insensitive personal style settings", async (t) => {
  const cwdRoot = await makeTempDir("wechat-style-cwd-");
  const homeRoot = await makeTempDir("wechat-style-home-");
  ...
  await writeExtendFile(
    ...
    [
      ...
      "DEFAULT_COLOR: green",
      "STYLE_FONT_SIZE: 17px",
      "STYLE_LINE_HEIGHT: 1.9",
      "STYLE_PARAGRAPH_SPACING: 1.2em 6px",
      "STYLE_HEADING: underline",
      "STYLE_BLOCKQUOTE: border",
      "STYLE_MAC_CODE_BLOCK: FALSE",
      "STYLE_SHOW_LINE_NUMBER: TRUE",
    ].join("\n"),
  );

  const style = resolveRenderStyle(loadWechatExtendConfig());
  assert.equal(style.profile, "personal");
  assert.equal(style.theme, "simple");
  assert.equal(style.color, "green");
  assert.equal(style.fontSize, "17px");
  assert.equal(style.lineHeight, "1.9");
  assert.equal(style.paragraphSpacing, "1.2em 6px");
  assert.equal(style.headingStyle, "underline");
  assert.equal(style.blockquoteStyle, "border");
  assert.equal(style.macCodeBlock, false);
  assert.equal(style.showLineNumber, true);
});
