...
async function checkBun(): Promise<void> {
  const direct = spawnSync('bun', ['--version'], { stdio: 'pipe', timeout: 10_000 });
  const result = direct.status === 0
    ? direct
    : spawnSync('npx', ['-y', 'bun', '--version'], { stdio: 'pipe', timeout: 30_000 });
  if (result.status === 0) {
...
async function checkEgoBrowser(): Promise<void> {
  ...
  const hasReadyMarker = output.includes('ego-browser-ready');
  const exitedCleanly = !result.error && result.signal === null;
  if (hasReadyMarker && exitedCleanly) {
    log('ego-browser', true, 'CLI and Node.js runtime are ready');
    return;
  }
  const detail = result.error?.message || result.stderr?.trim() || 'ego-browser CLI check failed';
  log('ego-browser', false, detail);
}

async function checkApiCredentials(): Promise<void> {
  const cwd = process.cwd();
  const projectEnv = path.join(cwd, '.kane-skills', '.env');
  const userEnv = path.join(os.homedir(), '.kane-skills', '.env');
  const legacyProjectEnv = path.join(cwd, '.baoyu-skills', '.env');
  const legacyUserEnv = path.join(os.homedir(), '.baoyu-skills', '.env');

  let found = false;
  for (const envPath of [projectEnv, userEnv, legacyProjectEnv, legacyUserEnv]) {
...
async function main(): Promise<void> {
  console.log('=== kane-post-to-wx: Permission & Environment Check ===\n');

  const legacy = process.argv.slice(2).includes('--legacy');

  await checkEgoBrowser();
  await checkBun();
  await checkApiCredentials();

  if (legacy) {
    console.log('\n--- Legacy Chrome CDP checks ---');
    await checkChrome();
