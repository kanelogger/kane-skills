...
function renderMarkdownWithPlaceholders(
  ...
  args.push("--style-profile", style.profile);
  args.push("--theme", style.theme);
  ...
  console.error(`[wechat-api] Rendering markdown with placeholders via md-to-wechat: ${style.profile}/${style.theme}${style.color ? `, color: ${style.color}` : ""}, citeStatus: ${citeStatus}`);
...
Options:
  ...
  --theme <name>      Theme name for markdown (default, grace, simple, modern)
...
async function main(): Promise<void> {
  ...
  const style = resolveRenderStyle(extConfig, {
    theme: args.theme,
    color: args.color,
  });
  ...
  if (args.isHtml) {
    ...
    console.error(`[wechat-api] Style: ${style.profile}/${style.theme}${style.color ? `, color: ${style.color}` : ""}, citeStatus: ${args.citeStatus}`);
    const rendered = renderMarkdownWithPlaceholders(filePath, style, args.citeStatus, args.title);
