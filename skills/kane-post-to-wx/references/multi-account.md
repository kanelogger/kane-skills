...
3. `.kane-skills/.env` with the prefixed key `WECHAT_{ALIAS}_APP_ID`
4. `~/.kane-skills/.env` with the prefixed key
...
```bash
# Account: kane
WECHAT_KANE_APP_ID=your_wechat_app_id
WECHAT_KANE_APP_SECRET=your_wechat_app_secret
...
The default ego-browser method uses an isolated task space and inherits the user's login state. The legacy Chrome CDP scripts still support per-account Chrome profiles:
...
${BUN_X} {baseDir}/scripts/wechat-ego-browser.ts --markdown <file> --theme default --account kane
${BUN_X} {baseDir}/scripts/wechat-ego-browser.ts --mode image-text --markdown <file> --images ./photos/ --account kane
```
...
${BUN_X} {baseDir}/scripts/wechat-api.ts <file> --theme default --account kane --remote --remote-host other-server.example.com
```
