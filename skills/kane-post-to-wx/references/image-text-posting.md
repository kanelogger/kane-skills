...
${BUN_X} ./scripts/wechat-ego-browser.ts --mode image-text --markdown source.md --images ./images/

# Post with explicit title and content
${BUN_X} ./scripts/wechat-ego-browser.ts --mode image-text --title "标题" --content "内容" --image img1.png --image img2.png

# Save as draft
${BUN_X} ./scripts/wechat-ego-browser.ts --mode image-text --markdown source.md --images ./images/ --submit
```
...
| `--resume` | Resume the handed-off ego-browser task space after confirmed login |
| `--task-space <name|id>` | Select the ego-browser task space |
...
4. Opens an isolated ego-browser task space and navigates to WeChat "贴图" editor
...
| Script | Purpose |
|--------|---------|
| `wechat-ego-browser.ts` | Default ego-browser image-text publisher |
| `wechat-browser.ts` | Legacy Chrome CDP image-text publisher |
