...
2. `<cwd>/.kane-skills/.env` with `WECHAT_APP_ID=...`
3. `$HOME/.kane-skills/.env` with `WECHAT_APP_ID=...`
...
Where to save?
A) Project-level: .kane-skills/.env (this project only)
B) User-level: ~/.kane-skills/.env (all projects)
```
...
Create the file with owner-only permissions (`chmod 600`) and ensure `.kane-skills/` is ignored by the current Git repository before writing project-level credentials. Never print the secret in command output or completion reports.
