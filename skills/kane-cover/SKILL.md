...
`kane-cover` is a self-contained cover-generation workflow with a skill-local style library. Select exactly one reusable style from `references/styles/`, then compile that style atom into the cover shape.
...
1. `kane-cover` task fields: platform, aspect ratio, optional output dimensions and output mode, title clarity, article summarization, cover communication goals, and universal output constraints.
...
- For the selected style, read both:
  - `references/styles/{style-id}/META.md`
  - `references/styles/{style-id}/STYLE.md`
- Only expose styles whose `META.md` metadata has `outputs` containing `cover` or `poster`.
- Do not expose photo, avatar, portrait, pet-only, polaroid-only, or image-remix styles in the `kane-cover` menu unless their metadata explicitly includes `cover` or `poster`.
...
5. Compile the final image prompt:
   ...
   - Read exactly one selected style visual atom from `references/styles/{style-id}/STYLE.md`.
   - Read the selected style metadata from `references/styles/{style-id}/META.md`, including `style_anchors`, `cover_shape_adaptation`, `must_preserve`, and `avoid_when_applying_to_cover` when present.
...
6. Save files before image generation:
   - For a single image, create `kane-assets/kane-cover/{slug}/prompts/cover.md` with the complete filled prompt.
   - For an explicitly requested multi-size suite, instead create one file per target as `kane-assets/kane-cover/{slug}/prompts/cover-{ratio-or-size}.md`, using filesystem-safe ratio or size labels.
   - For a single image, if image generation returns an explicit local file path, downloadable URL, or image bytes for the current run, save that artifact as `kane-assets/kane-cover/{slug}/cover.png`.
   ...
   - For a multi-size suite, save each explicitly retrievable artifact as `kane-assets/kane-cover/{slug}/cover-{ratio-or-size}.png`; never combine the suite into a contact sheet unless the user separately requests one.
...
```text
# kane-cover cover task instructions
