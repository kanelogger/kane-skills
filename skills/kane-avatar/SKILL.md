...
`kane-avatar` is a self-contained avatar-shape workflow with a skill-local style library. Select exactly one reusable style from `references/styles/`, then compile that style atom into the avatar shape.
...
1. `kane-avatar` task fields: subject type, source image or text description, intended use, likeness policy, avatar composition, aspect ratio, background simplification, crop safety, and universal output constraints.
...
- Expose only these seven styles in the `kane-avatar` menu:
  - `pixel-avatar`
...
- `surreal-pop-up-paper-landscape` has two output modes. After reading its `META.md` and `STYLE.md`, read exactly one mode reference selected by the user:
  - `references/styles/surreal-pop-up-paper-landscape/references/before-after.md`
  - `references/styles/surreal-pop-up-paper-landscape/references/final-artwork.md`
- Do not expose cover/poster styles in the `kane-avatar` menu.
- Do not read `kane-cover` references or templates for avatar runs.
...
2. Resolve aspect ratio:
   - Default ratio is always `1:1` inside `kane-avatar`.
   ...
   - For `polaroid-keepsake`, default to `1:1` in `kane-avatar`; only generate a vertical polaroid card when the user explicitly asks for a vertical ratio.
...
6. Compile the final image prompt:
   ...
   - Read the selected style metadata from `references/styles/{style-id}/META.md`.
...
7. Save files before image generation:
   - Create `kane-assets/kane-avatar/{slug}/prompts/avatar.md` with the complete filled prompt.
   - If image generation returns an explicit local file path, downloadable URL, or image bytes for the current run, save that artifact as `kane-assets/kane-avatar/{slug}/avatar.png`.
